 ...
 * @author wills
 */
public class ConnectDecoder extends BaseDecoder {
    ...
    void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws UnsupportedEncodingException {
        ...
        //int keepAlive = MQTTParseUtils.readWord(in);
        ...
        //Decode the ClientID
        String clientID = MQTTParseUtils.decodeString(in);
        if (clientID == null) {
            in.resetReaderIndex();
            return;
        }
        ...
        //Decode willTopic
        if (willFlag) {
            String willTopic = MQTTParseUtils.decodeString(in);
            if (willTopic == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode willMessage
        if (willFlag) {
            String willMessage = MQTTParseUtils.decodeString(in);
            if (willMessage == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode username
        if (userFlag) {
            String userName = MQTTParseUtils.decodeString(in);
            if (userName == null) {
                in.resetReaderIndex();
                return;
            }
        ...
        //Decode password
        if (passwordFlag) {
            String password = MQTTParseUtils.decodeString(in);
            if (password == null) {
                in.resetReaderIndex();
                return;
            }

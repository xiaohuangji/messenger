 ...
 * @author wills
 */
class PublishDecoder extends BaseDecoder {
    
    private final static Logger LOG = LoggerFactory.getLogger(PublishDecoder.class);
    ...
    void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        ...
        //Topic name
        String topic = MQTTParseUtils.decodeString(in);
        if (topic == null) {
            in.resetReaderIndex();
            return;
        }
        ...
        //read the payload
        int payloadSize = remainingLength - (stopPos - startPos - 2) + (MQTTParseUtils.numBytesToEncode(remainingLength) - 1);
